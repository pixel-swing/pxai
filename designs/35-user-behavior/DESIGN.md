...
version: "alpha"
id: "35-user-behavior"
source: "style-templates/pages/35-user-behavior.html"
description: "转化漏斗、用户旅程与留存队列的行为分析看板风格：漏斗逐级收窄、流失徽章告警、队列热力网格。"
colors:
...
# 35 用户行为分析 Design System

## 品牌与视觉方向

转化漏斗、用户旅程与留存队列的行为分析看板风格：漏斗逐级收窄、流失徽章告警、队列热力网格。

视觉关键词来自页面本身：用户在哪一步 离开了你；注册转化漏斗 · 近 30 天；风格特性；逐级漏斗；旅程流向；队列热力；按钮与控件；创建行为事件。界面应保持这些内容所体现的语气和信息层级，不要混入其他风格的装饰语言。
...
辅助字面色仅在令牌无法表达的局部状态或插画中使用：`#3B82F6`、`#DBEAFE`、`#93C5FD`、`#60A5FA`、`#1D4ED8`、`#22C55E`、`#15803D`、`#EF4444`、`#B91C1C`、`#F8FAFC`、`#FFFFFF`、`#E2E8F0`、`#0F172A`、`#475569`、`#64748B`、`#FFF`、`#2563EB`、`#94A3B8`、`#F1F5F9`、`#0891B2`、`#22D3EE`、`#083344`、`#FEE2E2`、`#1E3A8A`。
...
- `.hero h1 em`: font-style: normal; color: var(--retention-line-color).
- `.board h3`: font-size: 1rem; margin-bottom: 1rem.
...
圆角令牌：`--radius: 14px`。
空间令牌：`--funnel-width: 100%`。
阴影/海拔令牌：`--shadow: 0 1px 2px rgba(15,23,42,.05),0 10px 30px rgba(15,23,42,.07)`。
...
- `.card:nth-child(2) .ico`: background: #DCFCE7; color: #15803D.
- `.card:nth-child(3) .ico`: background: #FEE2E2; color: #B91C1C.
- `.card h3`: font-size: 1.05rem; margin-bottom: .4rem.
...
- `.nav ul a:hover`: color: var(--ink); border-color: var(--retention-line-color).
...
- `:focus-visible`: outline: 2px solid var(--retention-line-color); outline-offset: 2px; border-radius: 4px.
- `.nav ul a:hover`: color: var(--ink); border-color: var(--retention-line-color).
- `.btn-primary:hover`: background: #2563EB; box-shadow: 0 8px 20px rgba(37,99,235,.28).
...
- `:focus-visible`: outline: 2px solid var(--retention-line-color); outline-offset: 2px; border-radius: 4px.
- `input:focus-visible,textarea:focus-visible`: outline: 2px solid var(--retention-line-color); outline-offset: 1px; border-color: var(--retention-line-color).
