...
const outputDir = resolve(projectDir, 'src/styles/themes/generated')
...
function aliases(frontMatter, id) {
  ...
  if (sameColor(colors[surfaceKey], bgColor)) {
    const inkLum = luminance(colors[inkKey])
    const contrastColor = inkLum !== null && inkLum > 0.5 ? '#000000' : '#ffffff'
  ...
  const borderKey = pickToken(colorKeys, ['outline-subtle', 'border-subtle', 'divider', 'border', 'line', 'outline'], secondaryKey)
...
async function main() {
  ...
  const importLines = generated.map(({ id }) => `@import './generated/${id}.css';`)
